    ...
    internal static class moMapDrawingTools
    {
        ...
        private static void DrawSimpleMarker(Graphics g, Rectangle drawingArea, double dpm, moSimpleMarkerSymbol symbol)
        {
            ...
            { 
                // 绘制空心三角形
                Pen pen = new Pen(symbol.Color);
                PointF point1 = new PointF(drawingArea.Left + drawingArea.Width / 2, drawingArea.Top);
                PointF point2 = new PointF(drawingArea.Left, drawingArea.Bottom);
                PointF point3 = new PointF(drawingArea.Right, drawingArea.Bottom);
            ...
            { 
                // 绘制外层空心圆
                Pen outerPen = new Pen(symbol.Color);
                g.DrawEllipse(outerPen, drawingArea);
                outerPen.Dispose();

                // 计算内部实心点的绘制区域
                float innerDiameter = drawingArea.Width / 2; // 内部实心点的直径
                float innerLeft = drawingArea.Left + (drawingArea.Width - innerDiameter) / 2;
                float innerTop = drawingArea.Top + (drawingArea.Height - innerDiameter) / 2;
                RectangleF innerRect = new RectangleF(innerLeft, innerTop, innerDiameter, innerDiameter);

                // 绘制内部实心点
                SolidBrush innerBrush = new SolidBrush(symbol.Color);
            ...
            { 
                ...
                // 计算内部空心圆的绘制区域
                float innerDiameter = drawingArea.Width / 2; // 内部空心圆的直径
                float innerLeft = drawingArea.Left + (drawingArea.Width - innerDiameter) / 2;
                float innerTop = drawingArea.Top + (drawingArea.Height - innerDiameter) / 2;
                RectangleF innerRect = new RectangleF(innerLeft, innerTop, innerDiameter, innerDiameter);

                // 绘制内部空心圆
                Pen innerPen = new Pen(symbol.Color);
